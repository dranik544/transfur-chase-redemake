	...
	if body.is_in_group("player"):
		if !isopen:
			...
			match randomitem:
				...
				3:
					var enemy: CharacterBody3D = smallEnemyScene.instantiate()
					get_parent().add_child(enemy)
					enemy.set_physics_process(false)
					var tween = create_tween()
					enemy.global_position = global_position - Vector3(0, 1.0, 0)
					tween.tween_property(enemy, "global_position:y", 0.1, 1.0)
					await tween.finished
					enemy.set_physics_process(true)
					#enemy.global_position = global_position + Vector3(0, 0.8, 0)
